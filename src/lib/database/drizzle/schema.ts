import { pgTable, pgSchema, foreignKey, text, timestamp, uniqueIndex, boolean, doublePrecision, integer, index, primaryKey } from "drizzle-orm/pg-core"
...
export const subCategoryInErp = erp.table("sub_category", {
...
export const memberInErp = erp.table("member", {
	id: text().primaryKey().notNull(),
...
export const userInErp = erp.table("user", {
	id: text().primaryKey().notNull(),
	name: text().notNull(),
	email: text().notNull(),
...
export const invitationInErp = erp.table("invitation", {
	id: text().primaryKey().notNull(),
	organizationId: text().notNull(),
	email: text().notNull(),
	role: text(),
...
export const teamInErp = erp.table("team", {
	id: text().primaryKey().notNull(),
	name: text().notNull(),
	organizationId: text().notNull(),
	createdAt: timestamp({ precision: 3, mode: 'string' }).notNull(),
...
export const productInErp = erp.table("product", {
	id: text().primaryKey().notNull(),
	name: text().notNull(),
	code: text().notNull(),
	brand: text().notNull(),
...
export const sessionInErp = erp.table("session", {
	id: text().primaryKey().notNull(),
	expiresAt: timestamp({ precision: 3, mode: 'string' }).notNull(),
	token: text().notNull(),
	createdAt: timestamp({ precision: 3, mode: 'string' }).notNull(),
...
export const accountInErp = erp.table("account", {
	id: text().primaryKey().notNull(),
...
export const organizationInErp = erp.table("organization", {
	id: text().primaryKey().notNull(),
	name: text().notNull(),
	slug: text(),
	logo: text(),
...
export const categoryInErp = erp.table("category", {
	id: text().primaryKey().notNull(),
	name: text().notNull(),
	color: text().notNull(),
	organizationId: text().notNull(),
...
export const stockInErp = erp.table("stock", {
	id: text().primaryKey().notNull(),
	...
	foreignKey({
			...
			foreignColumns: [warehouseInErp.id],
			name: "stock_warehouseId_fkey"
		}).onUpdate("cascade").onDelete("restrict"),
]);

export const warehouseInErp = erp.table("warehouse", {
	id: text().primaryKey().notNull(),
	...
	foreignKey({
			columns: [table.organizationId],
			foreignColumns: [organizationInErp.id],
			name: "warehouse_organizationId_fkey"
		}).onUpdate("cascade").onDelete("restrict"),
]);
...
export const subCategoryProductsInErp = erp.table("_sub_category_products", {
	a: text("A").notNull(),
	b: text("B").notNull(),
}, (table) => [
...
export const providerProductsInErp = erp.table("_provider_products", {
	a: text("A").notNull(),
	b: text("B").notNull(),
}, (table) => [
	index().using("btree", table.b.asc().nullsLast().op("text_ops")),
